...
currency_codes = [company_currency,
                  'usd',
                  'krw']
...
for code in currency_codes:
    ...
    data = pd.concat([get_currency_data(code, year) for year in range(start_year, end_year + 1)])

    data.rename(columns={'effectiveDate': 'Date',
                'mid': f'{code}/pln'}, inplace=True)
...
for code in currency_codes[1:]:
    ...
    df_merged[f'{code}/{company_currency}'] = df_merged[f'{code}/pln'] / \
        df_merged[f'{company_currency}/pln']
...
dataset = df_merged.groupby(pd.PeriodIndex(
    df_merged['Date'], freq="M")).mean(numeric_only=True)
